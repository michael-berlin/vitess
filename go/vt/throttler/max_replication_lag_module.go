...
func (m *MaxReplicationLagModule) isReplicaUnderTest(r *result, now time.Time, testedState state, lagRecordNow replicationLagRecord) bool {
	...
	if now.Before(m.replicaUnderTest.nextAllowedChange) {
		r.Reason = fmt.Sprintf("skipping the lag record of this 'replica under test' because we're waiting %.1f more seconds to see if the lag changed", m.replicaUnderTest.nextAllowedChange.Sub(now).Seconds())
		return false
	}
