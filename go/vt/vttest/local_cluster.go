...
type Handle struct {
	...
	// We store it here because the VSchema() option is requiring it.
	schemaDir string
...
// VtgatePort returns the address under which vtgate is reachable e.g.
// "localhost:15991".
	...
	return VitessOption{
		beforeRun: func(hdl *Handle) error {
			...
			if _, err := os.Stat(vschemaFilePath); err == nil {
				return fmt.Errorf("cannot create temporary vschema.json file because it already exists at %v", vschemaFilePath)
			}
